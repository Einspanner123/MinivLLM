...
def decode_torch_optimized(
    ...
    padded_k = torch.zeros(
        batch_size, max_context_len, num_kv_heads, head_dim, device=device, dtype=dtype
    )
...
def setup_test_data(
    batch_size, seq_len, num_heads, num_kv_heads, head_dim, block_size, device="cuda"
):
    ...
    block_tables = torch.arange(total_blocks, device=device, dtype=torch.int32).reshape(
