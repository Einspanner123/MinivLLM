...
def naive_triton_attention_kernel(
    ...
    k_ptrs = (
        K
        + (seq_start + offs_m[:, None]) * num_kv_heads * head_dim
...
def naive_triton_attention(
    ...
    if max_seq_len > BLOCK_SIZE:
        print(
            f"      WARNING: seq_len ({max_seq_len}) > BLOCK_SIZE ({BLOCK_SIZE}), results may be incorrect"
        )
...
def find_crossover_point():
    ...
    for seq_len in seq_lengths:
        ...
        if naive_time:
            ...
            print(
                f"  Naive: {naive_time * 1000:.3f}ms | Flash: {flash_time * 1000:.3f}ms | Winner: {winner} ({abs(speedup):.2f}x)"
            )
            results.append((seq_len, naive_time, flash_time, winner))
        else:
            print(
                f"  Naive: SKIPPED | Flash: {flash_time * 1000:.3f}ms | Winner: Flash (by default)"
            )
    ...
    print("=" * 80)
    print(
        f"{'Seq Len':>10} | {'Naive (ms)':>12} | {'Flash (ms)':>12} | {'Winner':>10} | {'Speedup':>10}"
    )
    print("-" * 80)
    ...
    for seq_len, naive_time, flash_time, winner in results:
        if naive_time:
            speedup = (
                naive_time / flash_time
                if flash_time < naive_time
                else flash_time / naive_time
            )
        ...
        print(
            f"{seq_len:>10} | {naive_str:>12} | {flash_str:>12} | {winner:>10} | {speedup_str:>10}"
        )
...
def analyze_kernel_launches():
    ...
    print(
        f"  Extra overhead: ~{(flash_kernels - naive_kernels) * 10}μs = {(flash_kernels - naive_kernels) * 0.01:.2f}ms"
    )
...
def setup_data(num_seqs, seq_len, num_heads, num_kv_heads, head_dim):
    ...
    cu_seqlens = torch.tensor(
        [i * seq_len for i in range(num_seqs + 1)], device=device, dtype=torch.int32
    )
...
def benchmark(
    ...
    print(f"\n{'=' * 80}")
    print(
        f"Benchmark: {num_seqs} seqs × {seq_len} tokens (total: {num_seqs * seq_len} tokens)"
    )
    ...
    for _ in range(num_iter):
        outputs["pytorch"] = pytorch_standard_attention(
            q, k, v, cu_seqlens, scale, num_heads, num_kv_heads, head_dim
        )
    torch.cuda.synchronize()
    ...
    if seq_len <= max_safe_seq:
        ...
        for _ in range(5):
            _ = naive_triton_attention(
                q, k, v, cu_seqlens, scale, num_heads, num_kv_heads, head_dim, seq_len
            )

        torch.cuda.synchronize()
        start = time.perf_counter()
        for _ in range(num_iter):
            outputs["naive"] = naive_triton_attention(
                q, k, v, cu_seqlens, scale, num_heads, num_kv_heads, head_dim, seq_len
            )
        torch.cuda.synchronize()
        t = (time.perf_counter() - start) / num_iter
